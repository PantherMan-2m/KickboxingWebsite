...
export async function onRequestGet(context) {
  const { results } = await context.env.DB.prepare(
    `SELECT id, day_of_week, start_time, end_time, name, active
...
export async function onRequestPost(context) {
  ...
  await context.env.DB.prepare(
    `INSERT INTO class_templates (id, day_of_week, start_time, end_time, name) VALUES (?, ?, ?, ?, ?)`
  )
    .bind(id, dayOfWeek, startTime, body.endTime || null, name)
    .run();

  return jsonResponse({ ok: true, template: { id, dayOfWeek, startTime, endTime: body.endTime || null, name } });
}
