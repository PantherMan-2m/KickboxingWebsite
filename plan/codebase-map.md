...
| `api/student/payments.js` | **GET (T4.7, new)** | own plan (effective price), own `status`, own payment history -- derives the user from the session only; accepts no id parameter of any kind (IDOR-proof by construction, not by a check). History is `LIMIT 3` (most recent by `paid_on`/`created_at`), server-side -- older rows never leave the server. `status` is a **separate, unlimited** query (`paymentStatusForRoster`'s `MAX(covers_end)` spans every payment); the two queries must stay independent, or a student with 4+ payments could read `overdue` while an older, LIMIT-excluded payment still covers them |
